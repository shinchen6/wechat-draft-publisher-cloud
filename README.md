...
> **零三方依赖**：`publish_script.py` 只用 Python 标准库（`urllib` + `base64` + `json`）发请求；Markdown -> 微信图文 HTML 的转换也用标准库在本地完成（`md2wechat.py`）。只要环境有 Python 3.8+ 即可直接跑，无需 `pip install` 任何东西。
...
3. **config.json**（放在本 skill 目录，已被 `.gitignore` 忽略）：
   ```json
   { "relay_url": "https://<服务名>.<地域>.run.tcloudbase.com", "relay_key": "你的 RELAY_API_KEY" }
   ```

复制 `config.example.json` 为 `config.json` 填入即可。发布版**不硬编码**任何个人地址/密钥。
...
# 真发
python publish_script.py --real --article X.md --cover Y.png --titles-md T.md
```
