...
**在本 skill 本地完成**（`md2wechat.py`，零依赖纯标准库实现）。relay 不碰 Markdown。
...
3. **config.json**（放在本 skill 目录下，已被 `.gitignore` 忽略，不进仓库）：
   ```json
...
4. **零依赖发送 + 零依赖转换**：发送用标准库 `urllib` + `base64`（JSON 协议）；Markdown->HTML 用 `md2wechat.py`（纯标准库）。无需 `requests` / `curl` / `mistune`
